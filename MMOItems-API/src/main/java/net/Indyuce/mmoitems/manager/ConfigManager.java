...
public class ConfigManager implements Reloadable {
    ...
    private static final List<Runnable> CONFIG_UPDATES = ConfigVersioner.nops(8,
            MMOItems.plugin.getStats()::fixLegacyCustomStatIds,
            ConfigManager::rpgProviderConfigUpdate
    );

    public ConfigManager() {

        ConfigVersioner.applyConfigVersioner(MMOItems.plugin, CONFIG_UPDATES);
    ...
    private static void rpgProviderConfigUpdate() {
        var legacyKey = "preferred-rpg-provider";
        var miConfig = new YamlFile(MMOItems.plugin, "config");

        if (miConfig.getContent().contains(legacyKey)) {

            // Erase from MMOItems
            var rpgProviderName = miConfig.getContent().getString(legacyKey);
            miConfig.getContent().set(legacyKey, null);
            miConfig.save();

            // Write to MythicLib
            var mlConfig = new YamlFile(MythicLib.plugin, "config");
            mlConfig.getContent().set("class-plugin", rpgProviderName);
            mlConfig.getContent().set("level-plugin", rpgProviderName);
            mlConfig.getContent().set("mana-plugin", rpgProviderName);
            mlConfig.save();
        }
    }
    ...
    public boolean isAllowed(@NotNull String itemCommand) {
        if (this.itemCommandWhitelist.isEmpty()) return true;

        for (var pattern : this.itemCommandWhitelist)
            if (pattern.matcher(itemCommand).matches()) return true;
        return false;
    }
    ...
    public String getTriggerTypeName(@NotNull TriggerType triggerType) {
        var userDefined = triggerTypeNames.get(triggerType);
        if (userDefined == null) return triggerType.getName();
        return userDefined;
    }
